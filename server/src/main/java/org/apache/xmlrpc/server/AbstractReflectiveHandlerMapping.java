...
public abstract class AbstractReflectiveHandlerMapping implements
	...
	/**
	 * An object implementing this interface may be used to validate user names
	 * and passwords.
	 */
	public interface AuthenticationHandler {
		/**
		 * Returns, whether the user is authenticated and authorized to perform
		 * the request.
		 */
	...
	/**
	 * Sets the mappings {@link RequestProcessorFactoryFactory}. Note, that this
	 * doesn't affect already registered handlers.
	 */
	...
	/**
	 * Searches for methods in the given class. For any valid method, it creates
	 * an instance of {@link XmlRpcHandler}. Valid methods are defined as
	 * follows:
	 * <ul>
	 * <li>They must be public.</li>
	 * <li>They must not be static.</li>
	 * <li>The return type must not be void.</li>
	 * <li>The declaring class must not be {@link java.lang.Object}.</li>
	 * <li>If multiple methods with the same name exist, which meet the above
	 * conditins, then an attempt is made to identify a method with a matching
	 * signature. If such a method is found, then this method is invoked. If
	 * multiple such methods are found, then the first one is choosen. (This may
	 * be the case, for example, if there are methods with a similar signature,
	 * but varying subclasses.) Note, that there is no concept of the
	 * "most matching" method. If no matching method is found at all, then an
	 * exception is thrown.</li>
	 * </ul>
	 * 
	 * @param pKey
	 *            Suffix for building handler names. A dot and the method name
	 *            are being added.
	 * @param pType
	...
	protected void registerPublicMethods(String pKey, Class pType)
		...
		for (int i = 0; i < methods.length; i++) {
			...
			String name = ("root".equals(pKey)) ? method.getName() : pKey + "."
					+ method.getName();
			Method[] mArray;
	 ...
	 * 
	 * @param pClass
	 *            The class, which was inspected for handler methods. This is
	 *            used for error messages only. Typically, it is the same than
	 * 
	 *            <pre>
	 * pInstance.getClass()
	 * </pre>
	...
	protected XmlRpcHandler newXmlRpcHandler(final Class pClass,
		...
		RequestProcessorFactory factory = requestProcessorFactoryFactory
	...
	/**
	 * Creates a help string for the given method, when applied to the given
	 * class.
	 */
	...
	public String[][] getMethodSignature(String pHandlerName)
		...
		throw new XmlRpcNoSuchHandlerException(
				"No metadata available for method: " + pHandlerName);
	}

	/**
	 * Returns, whether void methods are enabled. By default, null values aren't
	 * supported by XML-RPC and void methods are in fact returning null (at
	 * least from the perspective of reflection).
	 */
	...
	/**
	 * Sets, whether void methods are enabled. By default, null values aren't
	 * supported by XML-RPC and void methods are in fact returning null (at
	 * least from the perspective of reflection).
	 */
